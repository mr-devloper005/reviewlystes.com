...
function SearchResultCard({ post, index }: { post: SitePost; index: number }) {
  ...
  return (
    <Link href={href} className={`editable-news-module editable-card-shine group block overflow-hidden rounded-md transition hover:-translate-y-1 hover:shadow-2xl ${strong ? 'md:col-span-2' : ''}`}>
      {image ? (
        <div className={`relative overflow-hidden bg-black ${strong ? 'aspect-[16/7]' : 'aspect-[16/10]'}`}>
          ...
          <span className="editable-kicker absolute left-4 top-4">{taskLabel}</span>
        </div>
      ) : null}
      <div className="p-5 sm:p-6">
        {!image ? <span className="editable-kicker">{taskLabel}</span> : null}
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#f7f7f5)] text-[var(--editable-page-text,#202125)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-12">
          <div className="editable-news-module grid gap-8 rounded-md p-6 backdrop-blur md:grid-cols-[0.8fr_1.2fr] lg:p-10">
            <div className="editable-animate-rise">
              <p className="editable-section-heading text-xs font-black uppercase tracking-[0.24em] text-[var(--slot4-accent)]">{pagesContent.search.hero.badge}</p>
            ...
            <form action="/search" className="editable-animate-soft-pop self-end rounded-md border border-[var(--editable-border)] bg-[#f7f7f5] p-4 sm:p-5">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                ...
                <select name="task" defaultValue={task} className="rounded-md border border-[var(--editable-border)] bg-white px-4 py-3 text-sm font-black outline-none">
                  ...
                  {enabledTasks.filter((item) => item.key === 'article').map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-12 w-full items-center justify-center rounded-md bg-[var(--editable-page-text,#2f1d16)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-page-bg,#fff7ee)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
