...
const taskDeck: Record<TaskKey, { icon: typeof FileText; archiveClass: string; promise: string; badge: string }> = {
  article: { icon: FileText, archiveClass: 'grid gap-5 md:grid-cols-2 xl:grid-cols-3', promise: 'Magazine cards highlight article images, headlines, summaries, and clear reading paths.', badge: 'Read' },
  ...
  return (
    <EditableSiteShell>
      <main style={archiveVars} className="bg-[var(--archive-bg)] text-[var(--archive-text)]">
        <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.05fr_0.7fr] lg:px-8 lg:py-12">
          <div className="editable-news-module editable-animate-rise rounded-md p-7 sm:p-10">
            <div className="editable-section-heading text-xs font-black uppercase tracking-[0.2em] text-[var(--archive-accent)]"><Icon className="h-4 w-4" /> {label}</div>
            ...
            <div className="mt-6 border-l-4 border-[var(--archive-accent)] bg-[#f7f7f5] p-4 text-sm font-bold leading-7 opacity-75">{deck.promise}</div>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={basePath} className="rounded-md bg-[var(--archive-text)] px-5 py-3 text-sm font-black text-[var(--archive-bg)]">Browse all</Link>
          ...
          <form action={basePath} className="editable-news-module editable-animate-soft-pop self-end rounded-md p-5 backdrop-blur">
            ...
            <button className="mt-3 h-12 w-full rounded-md bg-[var(--archive-text)] text-sm font-black text-[var(--archive-bg)]">Apply</button>
...
function ArticleArchiveCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  ...
  return (
    <Link href={href} className="editable-news-module editable-card-shine group overflow-hidden rounded-md transition hover:-translate-y-1 hover:shadow-xl">
      <div className="relative aspect-[4/3] overflow-hidden bg-black/5">
        <img src={image} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
        <span className="editable-kicker absolute left-4 top-4">{category}</span>
      </div>
      <div className="p-5">
