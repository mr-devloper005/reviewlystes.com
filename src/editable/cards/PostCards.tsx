...
export function EditorialFeatureCard({ post, href, label = 'Featured read' }: { post: SitePost; href: string; label?: string }) {
  return (
    <Link href={href} className={`editable-card-shine group block min-w-0 overflow-hidden ${dc.surface.dark} ${dc.motion.lift}`}>
      <div className="relative min-h-[520px] p-6 sm:p-8 lg:min-h-[620px]">
        ...
        <div className="relative z-10 flex h-full min-h-[460px] flex-col justify-end lg:min-h-[560px]">
          <span className="editable-kicker">{label}</span>
...
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`editable-news-module group ${dc.layout.minRailCard} block overflow-hidden ${dc.motion.lift}`}>
      <div className={`${dc.media.frame} ${dc.media.ratio}`}>
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" />
        <span className="editable-kicker absolute left-4 top-4">No. {String(index + 1).padStart(2, '0')}</span>
      </div>
      <div className="p-5">
...
export function CompactIndexCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`editable-news-module group block min-w-0 rounded-md p-5 ${dc.motion.lift}`}>
...
export function ArticleListCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} className={`editable-news-module group grid min-w-0 gap-5 overflow-hidden rounded-md p-4 ${dc.motion.lift} sm:grid-cols-[220px_minmax(0,1fr)]`}>
